...
- Commit locally as the work goes. Publishing follows the project's recorded authority profile: where one grants the working branch and its PR to an approved change, no separate instruction is needed for those; merging a PR, the release tag and a PRODUCTION deploy stay the owner's — a component's own stand is the session's routine where the profile says so. Where no profile is recorded, never push without an explicit owner instruction. Nothing non-English is pushed without recorded permission.
EOF
exit 0
